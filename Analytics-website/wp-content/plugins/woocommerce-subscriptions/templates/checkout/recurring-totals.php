			...
			<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
				...
				<tr class="cart-subtotal recurring-total">
					<?php if ( $display_th ) : $display_th = false; ?>
						<th rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php esc_html_e( 'Total', 'woocommerce-subscriptions' ); ?></th>
						<td data-title="<?php esc_attr_e( 'Total', 'woocommerce-subscriptions' ); ?>"><?php wcs_cart_totals_subtotal_html( $recurring_cart ); ?></td>
					<?php else : ?>
